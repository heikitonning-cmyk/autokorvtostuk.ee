...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const visible = jobs.filter((j: any) => { const t = new Date(j.start_planned).getTime(); return t >= now.getTime() - 86400000 && t <= end })
  const groups = new Map<string, any[]>()
  for (const job of visible) { const key = dateKey(job.start_planned); groups.set(key, [...(groups.get(key) ?? []), job]) }
