...
export default async function ManagerPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const view = query.view === 'all' ? 'all' : query.view === 'history' ? 'history' : 'active'
  const visibleJobs = view === 'all' ? jobs : jobs.filter((job: any) => view === 'history' ? isCompletedJob(job) : isActiveJob(job))
  const jobSections = managerJobSections(visibleJobs as any[], now)
  ...
  return <div className="page stack-lg">
    ...
    <section className="attention-grid">
      ...
      <div className="attention-card danger-soft"><span>Hilinenud</span><strong>{summary.overdueNotStarted.length}</strong><small>tähtaeg möödas ja lõpetamata</small></div>
    ...
    <section>
      <div className="section-title"><h2>{view === 'history' ? 'Tööde ajalugu' : view === 'all' ? 'Kõik tööd' : 'Aktiivsed tööd'} <span className="count">{visibleJobs.length}</span></h2><Link href="/manager/calendar">Ava kalender</Link></div>
      <div className="segmented"><Link className={view === 'active' ? 'active' : ''} href="/manager">Aktiivsed</Link><Link className={view === 'history' ? 'active' : ''} href="?view=history">Ajalugu</Link><Link className={view === 'all' ? 'active' : ''} href="?view=all">Kõik</Link></div>
