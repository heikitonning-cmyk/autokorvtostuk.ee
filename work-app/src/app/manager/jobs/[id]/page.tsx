...
import { cancelJob, confirmJob, deleteJob } from '../actions'
...
const correctionErrors: Record<string, string> = {
  ...
  'delete-job': 'Töö kustutamine ei õnnestunud.',
}
...
export default async function JobPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="page narrow stack-lg">
    ...
    {canMarkJobCompleted(user, job) && <MarkJobCompleted jobId={job.id} view="manager" />}
    <section className="detail-card stack"><div><h2>Kustuta töö</h2><p className="muted">Kustutab töö koos peatuste, fotokirjete ja ajalooga jäädavalt. Seda tegevust ei saa tagasi võtta.</p></div><DeleteJobForm action={deleteJob} jobId={job.id} jobName={jobName} buttonLabel="Kustuta töö jäädavalt" /></section>
  </div>
}
