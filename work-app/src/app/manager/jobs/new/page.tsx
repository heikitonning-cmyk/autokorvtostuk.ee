...
export default async function NewJobPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [{ customers, workTypes, vehicles }, pricing, params] = await Promise.all([getReferenceData(), getPricingSettings(), searchParams])
  ...
  return <div className="page narrow stack-lg">
    ...
    <form action={createJob} className="form-card stack">
      <label>Klient<select name="customerId" defaultValue=""><option value="">Klient määramata</option>{customers.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
